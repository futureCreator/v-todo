...
### v0.8.1 - 2026-04-06
- **Feature**: 뽀모도로 타이머 — 독립 5번째 탭 (할일 → 노트 → 타이머 → 위시 → 디데이)
- **Feature**: Apple Watch 스타일 원형 프로그레스 링 타이머 (25분 집중 → 5분 휴식 → 4회 후 15분 긴 휴식)
- **Feature**: 뽀모도로 통계 — 오늘 완료 수, 집중 시간, 연속일 요약 카드 + 30주 히트맵
- **Feature**: 타이머 종료 시 사운드 + 브라우저 알림 (Notification API)
- **Feature**: 뽀모도로 API (`/api/pomodoro`) — GET/POST 엔드포인트 및 JSON 파일 저장소
- **Fix**: 데일리 노트 "저장됨" 표시 중복 버그 수정
- **Fix**: 추가 버튼 위치/크기 통일 — 노트, 위시, 디데이 탭 모두 하단 고정, 동일한 마진 적용
- **Fix**: 뽀모도로 리셋 버튼 아이콘 정렬 및 크기 수정
